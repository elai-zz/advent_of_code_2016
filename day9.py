class Decompress:
	...
	def repeat(self, times, segment):
		self.result += times * list(segment)

	# index is the index of the start of the marker (not the open parens)
	# not in a data section
	def parse_marker(self, index):
		data_section_length = ""
		times = ""
		is_times = False
		while index < len(self.compressed):
			if self.compressed[index] == "x":
				is_times = True
			elif self.compressed[index] == ")":
				return int(data_section_length), int(times), index + 1 
			elif is_times:
				times += self.compressed[index]
			else:
				data_section_length += self.compressed[index]
			index += 1

	def parse_string(self):
		index = 0
		while index < len(self.compressed):
			if self.compressed[index] == "(":
				length, times, next_index = self.parse_marker(index + 1)
				repeated_section = self.compressed[next_index:next_index+length]
				self.result += times * list(repeated_section)
				index = next_index+length
			else:
				self.result.append(self.compressed[index])
				index += 1


	def has_marker(self):
		if self.compressed.find("(") > -1:
			return True
		else:
			return False

	def keep_decompressing(self):
		while self.has_marker():
			self.parse_string()
			self.compressed = ("").join(self.result)
		return
...
class Phrase():
	...
	def get_marker_times_and_length():
		index = 0
		times = ""
		is_times = False
		while index < len(self.string):
			if self.compressed[index] == "x":
				...
				marker_length = index
				return int(times), marker_length
			if is_times:
				times += self.compressed[index]
			index += 1

	def remove_marker():
		repeat, marker_length = self.get_marker_times_and_length()
		self.phrase = self.phrase[marker_length+1:]

class LargeStringChopper():
	def __init__(self, string):
		self.compressed = string
		self.index = 0
		self.helper = Helper()
	...
	def get_next_chunk(self):
		...
		if self.compressed == "":
			...
			first_x = self.compressed.find("x")
			first_close_parens = self.compressed.find(")")
			marker_length = first_close_parens + 1
			length_of_data = int(self.compressed[1:first_x])
			chunk = self.compressed[self.index:marker_length+length_of_data]
			self.compressed = self.compressed[marker_length+length_of_data:]
		else:
			index = 0
			while index < len(self.compressed) and self.compressed[index] != "(":
				chunk += self.compressed[index]
				index += 1
			self.compressed = self.compressed[index:]
		yield chunk

	def get_total_length(self):
		chunk = next(self.get_next_chunk())
		while chunk != "":
			print chunk
			self.total+= self.helper.test_recursion(chunk)
			chunk = next(self.get_next_chunk())
		return self.total

class Helper():
	def remove_marker_on_chunk(self, chunk):
		index = 0
	...
	def is_lowest_form(self, chunk):
		if chunk.find("(") == -1:
			return True
		else:
			return False 

	def get_marker_times(self, chunk):
		index = chunk.find("x")+1
		times = ""
		while index < len(chunk):
			if chunk[index] == ")":
				return int(times)
	...
	def test_recursion(self, chunk):
		if self.is_lowest_form(chunk):
			return len(chunk)
		else:
			total = 0
			times = self.get_marker_times(chunk)
			chunk = self.remove_marker_on_chunk(chunk)
			large_string = LargeStringChopper(chunk)
			chopped = next(large_string.get_next_chunk())
			while chopped != "":
				total += self.test_recursion(chopped)
				chopped = next(large_string.get_next_chunk())
			return times * total
...
print largeString.get_total_length()
